...
export async function POST(request: NextRequest) {
  ...
  const { messageId, body } = (await request.json().catch(() => ({}))) as { messageId?: string; body?: string };
  if (!messageId || !body?.trim()) return NextResponse.json({ error: 'Missing reply content.' }, { status: 400 });
  ...
  try {
    ...
    if (!originalResponse.ok) return NextResponse.json({ error: 'Could not load the original message.' }, { status: 502 });
    const original = (await originalResponse.json()) as { threadId?: string; payload?: { headers?: GmailHeader[] } };
    ...
    const mime = [
      ...
      originalMessageId ? `References: ${references ? `${references} ` : ''}${originalMessageId}` : '',
      'MIME-Version: 1.0',
    ...
    if (!sendResponse.ok) {
      console.error('[gmail] reply send failed', sendResponse.status, await sendResponse.text());
