...
function extractBody(
  ...
  const paragraphs = text
    ...
    .slice(0, 8)
    .map((paragraph) =>
      paragraph.length > 700 ? `${paragraph.slice(0, 700)}…` : paragraph
    );
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const { id } of list.messages ?? []) {
      const detailResponse = await gapi(
    ...
    if (refreshedToken) {
      response.cookies.set(
        'gmail_connection',
        seal({ ...connection, accessToken: refreshedToken }),
        {
          httpOnly: true,
          maxAge: 60 * 60 * 24 * 30,
          path: '/',
