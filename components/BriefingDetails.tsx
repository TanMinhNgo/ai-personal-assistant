...
function classify(platform: string, message: InboxMessage): CategoryKey[] {
  const cats: CategoryKey[] = [
    EMAIL_PLATFORMS.has(platform) ? 'email' : 'messages',
  ];
  ...
  if (!message.preview.startsWith('You:') && tags.includes('UNREAD'))
    cats.push('followups');
  return cats;
}
...
function flatten(items: GatheredItems): FlatItem[] {
  return items.flatMap((item) =>
    item.messages.map((message) => ({
      ...
      message,
      categories: classify(item.platform, message),
    }))
  );
}
...
async function generateTop(
  items: GatheredItems
): Promise<BriefingResult['top'] | null> {
...
export function BriefingDetails({ id }: { id: string }) {
    ...
    async function run() {
        ...
        if (user) {
          ...
          try {
            ...
            if (
              ...
              storedTop =
                parseJson<{ top?: BriefingResult['top'] }>(row.data, {}).top ??
                null;
            }
          } catch {
      ...
      try {
        const { data } = await insforge.database
          ...
          .eq('id', id);
        const row = (data ?? [])[0] as
          { title?: string; data?: unknown } | undefined;
        if (cancelled) return;
        if (row) {
  ...
  const filtered = useMemo(
    () =>
      activeCategory === 'all'
        ? flat
        : flat.filter((entry) => entry.categories.includes(activeCategory)),
    [flat, activeCategory]
  );
      ...
      {notFound ? (
        <div className="dashboard-card mt-4 grid place-items-center rounded-3xl border border-slate-200 bg-white p-12 text-center">
          ...
          <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
            This briefing may have been removed, or the database isn&apos;t set
            up yet. Head back and open today&apos;s briefing instead.
          </p>
          <Link
            href="/briefing"
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-violet-500 px-5 py-3 text-sm font-bold text-white transition hover:bg-violet-400"
          >
            Back to briefings
          </Link>
        </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <span
                            className={`grid size-9 shrink-0 place-items-center overflow-hidden rounded-xl ${meta?.tileClass ?? 'bg-slate-100'}`}
                          >
                            <Image
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 text-sm leading-5 text-slate-600">
                              {entry.message.body.join(' ').slice(0, 240)}
                            </p>
                            <button
                              type="button"
                            ...
                            >
                              ...
                              {EMAIL_PLATFORMS.has(entry.platform)
                                ? 'Reply by email'
                                : 'Reply with message'}
                            </button>
                          </div>
...
function ComposePanel({
  ...
  if (targetKey !== lastTarget.current) {
    ...
    setSubject(
      target && EMAIL_PLATFORMS.has(target.platform)
    ...
    return (
      <div className="dashboard-card rounded-3xl border border-slate-200 bg-white p-6">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Pick an item and choose reply to draft and send a response with AI
          help.
        </p>
      </div>
    );
  ...
  async function aiDraft() {
    ...
    try {
      ...
      if (!response.ok)
        setStatus({ kind: 'error', text: data.error ?? 'Could not draft.' });
  ...
  async function send() {
    ...
    try {
      if (!isEmail) {
        ...
        setStatus({
          kind: 'ok',
          text: 'Draft copied — paste it into the app to send.',
        });
        setSending(false);
        return;
      }
      ...
      if (response.ok && data.ok)
        setStatus({ kind: 'ok', text: 'Sent successfully.' });
    ...
    <div className="dashboard-card rounded-3xl border border-slate-200 bg-white p-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold tracking-tight">
          {isEmail ? 'Reply by email' : 'Reply with message'}
        </h3>
        <button
          type="button"
          onClick={onClose}
      ...
      <p className="mt-1 text-sm text-slate-500">
        To <span className="font-semibold text-slate-700">{recipient}</span> ·
        via {platformName(target.platform)}
      </p>
      ...
      <div className="mt-4 space-y-3">
        ...
        <input
          ...
          placeholder="What do you want to say? (for AI draft)"
          className={inputClass}
        />
        <textarea
          value={body}
        ...
        {status && (
          <p
            className={`rounded-xl px-4 py-2.5 text-sm ${status.kind === 'ok' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}
          >
            {status.text}
          </p>
        )}
