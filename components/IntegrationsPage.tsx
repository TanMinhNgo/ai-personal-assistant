...
const integrations: Integration[] = [
  {
    ...
    name: 'Gmail',
    logo: '/gmail.png',
    description: 'Read, organize, and draft messages from your Gmail inbox.',
    actions: [
  ...
  },
  {
    ...
    logo: '/slack.png',
    description:
      'Bring channel updates and direct messages into your daily brief.',
    actions: [
  ...
  },
  {
    ...
    name: 'Outlook',
    logo: '/email.png',
    description: 'Coordinate your inbox and calendar from Microsoft Outlook.',
    actions: [
      'Read inbox messages',
  ...
  },
  {
    ...
    logo: '/discord.png',
    description:
      'Track community conversations without missing key discussions.',
    actions: [
  ...
  },
  {
    ...
    logo: '/linkedin.png',
    description:
      'Keep up with professional messages and important networking updates.',
    actions: [
      'Review messages',
  ...
  },
  {
    ...
    name: 'Telegram',
    logo: '/telegram.png',
    description: 'Surface the Telegram chats that need your attention first.',
    actions: [
      'Review chats',
      'Search messages',
  ...
  },
  {
    ...
    logo: '/globe.svg',
    description:
      'Connect another tool through a custom integration or webhook.',
    actions: [
      'Configure webhook',
      'Set custom actions',
  ...
  async function toggleConnection(platform: string) {
    ...
    if (platform === 'gmail' && connections.gmail !== 'connected') {
      ...
      authorizationUrl.search = new URLSearchParams({
        ...
        response_type: 'code',
        scope:
          'https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.compose https://www.googleapis.com/auth/gmail.send',
        access_type: 'offline',
        prompt: 'consent',
        state: userId,
      }).toString();
    ...
    const nextStatus: ConnectionStatus =
      connections[platform] === 'connected' ? 'disconnected' : 'connected';
    const hasRecord = Object.prototype.hasOwnProperty.call(
      connections,
      platform
    );
    ...
    const { error } = hasRecord
      ...
      : await insforge.database
          .from('user_integrations')
          .insert([
            { user_id: userId, platform, status: nextStatus, updated_at: now },
          ]);

    if (!error)
      setConnections((current) => ({ ...current, [platform]: nextStatus }));
    setUpdatingPlatform(null);
  }
      ...
      <div className="mx-auto max-w-7xl p-5 sm:p-8">
        <section className="flex flex-col justify-between gap-5 rounded-3xl bg-slate-950 p-6 text-white sm:flex-row sm:items-end sm:p-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">
              Bring the platforms you use every day into OmniMind and decide
              exactly what the assistant can do with each connection.
            </p>
          </div>
        ...
        <section className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          {integrations.map((integration) => {
            ...
            const isUpdating = updatingPlatform === integration.id;
            return (
              <article
                key={integration.id}
              ...
              >
                ...
                <p className="mt-2 min-h-11 text-sm leading-5 text-slate-500">
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <button
                    type="button"
                    disabled={!userId || Boolean(updatingPlatform)}
                    onClick={() => void toggleConnection(integration.id)}
                    className={`flex-1 rounded-xl px-4 py-3 text-sm font-bold transition active:translate-y-px disabled:cursor-not-allowed disabled:opacity-50 ${isConnected ? 'bg-rose-50 text-rose-700 hover:bg-rose-100' : 'bg-slate-950 text-white hover:bg-slate-800'}`}
                  >
                    {isUpdating
                      ? 'Saving...'
                      : isConnected
