...
export function CustomBriefingDialog({ userId, connected, items, onClose, onCreated }: Props) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [apps, setApps] = useState<string[]>(connected);
  const [categories, setCategories] = useState<CategoryKey[]>([...CATEGORY_KEYS]);
  ...
  async function save() {
    ...
    try {
      ...
      const { error: insertError } = await insforge.database.from('briefing_schedules').insert([
        {
          user_id: userId,
          name: name.trim(),
      ...
      if (generateNow && items.length > 0) {
        const filtered = apps.length ? items.filter((item) => apps.includes(item.platform)) : items;
        const response = await fetch('/api/briefing/generate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ items: filtered, goal: description.trim(), categories }),
        });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" role="presentation" onClick={onClose}>
      ...
      >
        <header className="flex items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
          <h2 id="custom-briefing-title" className="text-lg font-bold tracking-tight">
            Create custom briefing
          </h2>
          <button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-950" aria-label="Close">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-6">
          <Field label="Briefing name">
            <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Morning work digest" className={inputClass} autoFocus />
          </Field>

          <Field label="Description or goal">
            <textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={2} placeholder="Summarize urgent client emails and unread team chats." className={inputClass} />
          </Field>
              ...
              {PLATFORMS.filter((platform) => platform.id !== 'other').map((platform) => {
                ...
                return (
                  <button
                    ...
                    title={isConnected ? undefined : 'Connect this app on the Integrations page first'}
                    onClick={() => isConnected && setApps((list) => toggle(list, platform.id))}
                  ...
                  >
                    <Image src={platformLogo(platform.id)} alt="" width={18} height={18} className={`size-4 object-contain ${isConnected ? '' : 'grayscale'}`} />
                    {platform.name}
                    {!isConnected && <span className="text-[10px] font-medium text-slate-400">· not connected</span>}
                  </button>
                );
              })}
            </div>
            {connected.length === 0 && <p className="mt-2 text-xs text-slate-500">No apps connected yet — connect one on the Integrations page to include it.</p>}
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Frequency">
              <select value={frequency} onChange={(event) => setFrequency(event.target.value as Frequency)} className={inputClass}>
          ...
          <Field label="Priority">
            <select value={priority} onChange={(event) => setPriority(event.target.value as 'high' | 'medium' | 'low')} className={inputClass}>
          ...
          <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
            <input type="checkbox" checked={generateNow} onChange={(event) => setGenerateNow(event.target.checked)} className="size-4 rounded border-slate-300 text-violet-600" />
        ...
        <footer className="flex justify-end gap-2 border-t border-slate-200 px-6 py-4">
          <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-700 transition hover:bg-slate-50">
            Cancel
          </button>
          <button
            type="button"
