...
export function DashboardHome() {
  ...
  async function load(force: boolean) {
    ...
    try {
      ...
      const data = (await response.json().catch(() => ({}))) as BriefResult & { error?: string };
      if (!response.ok) {
      ...
      {noConnections ? (
        ...
        <>
          <section className="mt-6 grid gap-4 sm:grid-cols-3">
            <StatCard label="Important" value={result?.stats.important} loading={busy} icon={Star} color="bg-violet-500" />
            <StatCard label="Priority" value={result?.stats.priority} loading={busy} icon={Bell} color="bg-rose-500" />
            <StatCard label="Follow-ups" value={result?.stats.followUps} loading={busy} icon={Clock} color="bg-amber-400 text-amber-950" />
          </section>

          <section className="mt-6 grid gap-4 lg:grid-cols-[1.6fr_1fr]">
            <TodaysBrief items={result?.brief ?? []} loading={busy} error={error} onRefresh={() => void load(true)} />
            <div className="space-y-4">
              <ConnectedAppsCard connected={connected} />
              <PriorityItemsCard items={result?.priorities ?? []} loading={busy} />
            </div>
          </section>
        </>
      )}
  ...
  async function gatherItems(userId: string, connectedIds: string[]) {
    ...
    for (const id of connectedIds) {
      ...
      else if (id === 'whatsapp') messages = await fetchMessages(`/api/whatsapp/messages?userId=${encodeURIComponent(userId)}`);
...
function readCache(key: string, ids: string[]): BriefResult | null {
  ...
  try {
    ...
    if (!raw) return null;
    const cached = JSON.parse(raw) as { ts: number; data: BriefResult; platforms: string[] };
...
function writeCache(key: string, data: BriefResult, ids: string[]) {
  ...
  try {
    localStorage.setItem(key, JSON.stringify({ ts: Date.now(), data, platforms: ids }));
  } catch {
    /* storage unavailable */
  }
}
...
function StatCard({ label, value, loading, icon: Icon, color }: { label: string; value?: number; loading: boolean; icon: LucideIcon; color: string }) {
  return (
    <div className="dashboard-card rounded-3xl border border-slate-200 bg-white p-5">
      ...
      <p className="mt-5 text-3xl font-bold tracking-tight">{loading && value === undefined ? '—' : (value ?? 0)}</p>
...
function TodaysBrief({ items, loading, error, onRefresh }: { items: Brief[]; loading: boolean; error: string | null; onRefresh: () => void }) {
  return (
    <div className="dashboard-card rounded-3xl border border-slate-200 bg-white p-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold tracking-tight">Today&apos;s Brief</h3>
      ...
      <div className="mt-5 space-y-3">
        {error ? (
          ...
          <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No brief items right now. Check back after new messages arrive.</p>
        ) : (
              ...
              <div key={index} className="flex items-start gap-3 rounded-2xl bg-slate-50 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <Icon size={15} className="shrink-0 text-violet-600" aria-hidden="true" />
...
function PriorityItemsCard({ items, loading }: { items: Priority[]; loading: boolean }) {
  ...
  return (
    <div className="dashboard-card mt-6 grid place-items-center rounded-3xl border border-slate-200 bg-white p-12 text-center">
      ...
      <p className="mt-2 max-w-md text-sm leading-6 text-slate-500">
        Link Gmail, WhatsApp, and more so OmniMind can gather today&apos;s messages and generate a personalized brief for you.
      </p>
