...
function unseal<T>(value: string): T | null {
  try {
    ...
    const decipher = createDecipheriv(algorithm, getKey(), payload.subarray(0, 12));
...
export async function loadEncryptedAuthState(userId: string) {
  ...
  const persist = () => {
    writing = writing.then(() => fs.writeFile(file, seal({ creds, keys }))).catch(() => {});
    return writing;
  };
    ...
    keys: {
      ...
      set: async (data) => {
        for (const type of Object.keys(data)) {
          keys[type] = keys[type] ?? {};
          Object.assign(keys[type], (data as Record<string, Record<string, unknown>>)[type]);
        }
        await persist();
      },
    },
