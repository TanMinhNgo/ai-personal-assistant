...
const globalRef = globalThis as unknown as { __waSessions?: Map<string, WASession> };
...
async function spawnSocket(userId: string, phone: string): Promise<WASocket> {
  ...
  sock.ev.on('connection.update', (update) => handleConnection(userId, phone, update));
  sock.ev.on('messages.upsert', ({ messages }) => ingest(userId, messages));
  sock.ev.on('messaging-history.set', ({ messages }) => ingest(userId, messages));
  return sock;
}

function handleConnection(userId: string, phone: string, update: Partial<ConnectionState>) {
  const session = sessions.get(userId);
  if (!session) return;
...
export async function startPairing(userId: string, rawPhone: string) {
  ...
  if (existing?.status === 'connected') return { status: 'connected' as WAStatus, pairingCode: null, error: null };
  if (existing?.starting) return { status: existing.status, pairingCode: existing.pairingCode, error: existing.error };
  ...
  return { status: session.status, pairingCode: session.pairingCode, error: session.error };
}

export function getStatus(userId: string) {
  const session = sessions.get(userId);
  if (!session) return { status: 'idle' as WAStatus, pairingCode: null, error: null };
  return { status: session.status, pairingCode: session.pairingCode, error: session.error };
}
...
export async function ensureRunning(userId: string) {
  ...
  const auth = await loadEncryptedAuthState(userId);
  if (!auth.state.creds.registered) return { status: 'idle' as WAStatus, pairingCode: null, error: null };
  const phone = (auth.state.creds.me?.id ?? '').split(/[:@]/)[0].replace(/\D/g, '');
  const session: WASession = {
    sock: null,
    status: 'pairing',
    pairingCode: null,
...
function ingest(userId: string, messages: WAMessage[]) {
  ...
  for (const message of messages) {
    ...
    session.messages.set(id, { msg: message, ts: Number(message.messageTimestamp ?? 0) });
  }
  // Keep the buffer bounded — drop the oldest beyond ~500 entries.
  if (session.messages.size > 500) {
    const oldest = [...session.messages.entries()].sort((a, b) => a[1].ts - b[1].ts).slice(0, session.messages.size - 500);
...
function toInboxMessage(message: WAMessage, ts: number): InboxMessage | null {
  ...
  const timeLabel = ts ? new Date(ts * 1000).toLocaleString([], { hour: '2-digit', minute: '2-digit', month: 'numeric', day: 'numeric' }) : '';
  ...
  return {
    ...
    preview: message.key?.fromMe ? `You: ${text}`.slice(0, 120) : text.slice(0, 120),
    time: timeLabel,
