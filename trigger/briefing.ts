...
function db() {
  ...
  if (!baseUrl || !key)
    throw new Error('Missing INSFORGE_URL / key env for the briefing task.');
...
export const generateBriefing = task({
  ...
  run: async (payload: GeneratePayload) => {
    ...
    const { data: scheduleRows } = await client.database
    ...
    const { data: snapshotRows } = await client.database
    ...
    let result: Record<string, unknown> = {
      top: {
        ...
        summary: 'No fresh items in the latest snapshot.',
        highlights: [],
      },
      categories: [],
    };
    ...
    if (apiKey && items.length > 0) {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: buildBriefingPrompt(
          items,
          String(schedule.description ?? ''),
          asArray(schedule.categories)
        ),
        config: {
...
export const scanBriefingSchedules = schedules.task({
  ...
  run: async () => {
    ...
    await generateBriefing.batchTrigger(
      rows.map((row) => ({
        payload: { scheduleId: row.id, userId: row.user_id },
      }))
    );
